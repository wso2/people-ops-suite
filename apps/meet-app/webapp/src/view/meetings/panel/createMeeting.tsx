...
const validationSchema = yup.object({
  ...
  startTime: yup
    ...
    .test("is-future-time", "Start time must be in the future", function (value) {
      ...
      const selectedStartDateTime = dayjs.tz(`${date.format("YYYY-MM-DD")}T${value.format("HH:mm")}`, timeZone);
  ...
  endTime: yup
    ...
    .test("is-after-startTime", "End time must be after start time", function (value) {
      ...
      const start = dayjs.tz(`${date.format("YYYY-MM-DD")}T${startTime.format("HH:mm")}`, timeZone);
      const end = dayjs.tz(`${date.format("YYYY-MM-DD")}T${value.format("HH:mm")}`, timeZone);
...
function MeetingForm() {
  ...
  return (
    <>
      ...
      {employeeState === State.failed ? (
        <ErrorHandler message="Something went wrong! Please try again later." />
      ) : (
        <>
            ...
            >
              ...
              <Box sx={{ display: "flex", gap: 2, pb: 0.5 }}>
                <Autocomplete
                  ...
                  onChange={async (_, tz) =>
                    await formik.setFieldValue("timeZone", tz ?? Intl.DateTimeFormat().resolvedOptions().timeZone)
                  }
                  renderInput={(params) => (
                    <TextField
                      {...params}
              ...
              <Autocomplete
                ...
                onChange={async (_, newValue) => {
                  const emails = newValue
                    ...
                    .filter(
                      (email, index, self): email is string => email !== undefined && self.indexOf(email) === index
                    );
